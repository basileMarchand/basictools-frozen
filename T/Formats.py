...
class BaseTensor(object):
    ...
    def Restriction(self,part={}):
        ...
        for c in res.cores:
            index = []
            for n,cpt in zip(c.names,range(c.ndims)):
                
                if part.keys().count(n) :
                    #c.array = c.array.slice()
                    index.append(slice(part[n],part[n]+math.copysign(1,part[n]), math.copysign(1,part[n]) ) )
                else:
...
class SparseBaseTensor(BaseTensor):
    ...
    def Reduction(self,redRanks=[]):
        ...
        #print(spaces)
        dictionary = dict(zip(spaces, dic[0:len(spaces)]))
...
def CheckIntegrity():
    ...
    # Test SparceBaseTensor CheckIntegrity    
